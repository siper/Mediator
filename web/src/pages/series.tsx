...
export function SeriesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["media", "by-type", MediaType.Series],
    queryFn: () =>
      api.get<Media[]>(`/api/media?type=${MediaType.Series}&limit=100`),
  });

  return (
    <div className="mx-auto max-w-7xl px-8 py-8">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight">{t("pages.series.title")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("pages.series.description")}
        </p>
      </header>
